...
const ManagerTaskView = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      ...
      try {
        ...
        // Sorting tasks by the latest createdAt first
        const sortedTasks = Array.isArray(response.data)
      ...
      setTaskDetails((prev) => ({
        ...
        [taskId]: {
          ...
          comments: [
            ...
            { user_id: { name: decoded.name }, comment: commentText[taskId] },
          ],
        },
      }));
  ...
  return (
    <>
      {role === "Admin" ? <ProjectNavbar /> : <Navbar />}
      <div className="manager-container">
        <div className="manager-card">
          <h2>{role === "Admin" ? "Admin Task View" : "Manager Task View"}</h2>
              ...
              ? tasks.map((task) => {
                  ...
                  return (
                    <div key={task._id} className={`task-card ${priorityClass}`}>
                      ...
                      <p>
                        <strong>Priority:</strong> {task.priority || "Low"}
                      </p>
                      ...
                      {expandedTask === task._id && taskDetails[task._id] && (
                        <div className="task-details">
                          ...
                          <p>
                            ...
                            {new Date(taskDetails[task._id].deadline).toDateString()}
                          </p>
                          <p>
                            <strong>Assigned To:</strong>{" "}
                            {taskDetails[task._id].assigned_to?.name || "Unknown"}
                          </p>
